...
export const InfiniteMovingCards = <T,>({
	...
	return (
		...
		>
			...
			>
				{items.map((item, idx) => (
					<li className="relative shrink-0" key={`scroller-item-${idx}`}>
						{renderItem ? (
							renderItem(item, idx)
						) : (
							<div className="w-[350px] max-w-full rounded-2xl border border-b-0 border-zinc-200 bg-[linear-gradient(180deg,#fafafa,#f5f5f5)] px-8 py-6 md:w-[450px] dark:border-zinc-700 dark:bg-[linear-gradient(180deg,#27272a,#18181b)]">
								<blockquote>
									<div
										aria-hidden="true"
